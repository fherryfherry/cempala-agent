...
def test_oversized_line_is_skipped_and_run_continues(tmp_path, monkeypatch):
    monkeypatch.setattr(settings, "AGY_STREAM_LIMIT_BYTES", 1024)
    big = "x" * 8192
    script = _write_script(
        tmp_path / "agy",
        f"""printf '{{"event": "step_update", "step_update": {{"step_type": "agent_response", "text_delta": "{big}"}}}}\\n'
printf '{{"event": "result", "result": {{"status": "SUCCESS"}}}}\\n'
""",
    )
    monkeypatch.setattr(settings, "AGY_BIN", script)
...
def test_num_handles_bad_values():
    from app.agents.agy_tool import _num
...
def test_non_dict_json_line_skipped(tmp_path, monkeypatch):
    script = _write_script(
        ...
        r"""
printf '[1, 2, 3]\n'
printf '{"event": "result", "result": {"conversation_id": "sess-abc", "status": "SUCCESS", "response": "hi", "num_turns": 1, "usage": {"input_tokens": 1, "output_tokens": 1}}}\n'
""",
    )
    monkeypatch.setattr(settings, "AGY_BIN", script)
...
def test_terminate_noop_when_process_already_exited(tmp_path):
    async def _run():
        ...
        await proc.wait()
        await AgyTool._terminate(proc)  # must return immediately, no terminate()/kill()

    asyncio.run(_run())


def test_cancel_kills_process_that_ignores_sigterm(tmp_path, monkeypatch):
    import app.agents.agy_tool as agy_tool_mod

    monkeypatch.setattr(agy_tool_mod, "_TERMINATE_GRACE_SECONDS", 0.2)
    sleep_marker = f"sleep {__import__('random').randint(100000, 999999)}"
    script = _write_script(
        tmp_path / "agy", f"trap '' TERM\nexec {sleep_marker}\n"
    )
    monkeypatch.setattr(settings, "AGY_BIN", script)
